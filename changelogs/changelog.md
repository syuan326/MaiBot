...
- 恢复并内置鉴权器：对 Planner 决策与 Replyer 回复做身份核对，防止麦麦认错用户；核对不通过时注入驳回反馈并强制重新规划/重新生成，重试耗尽后放弃发送。
- 支持固定身份规则（UID 硬比对）：将指定用户绑定为固定身份，专属称呼与各群群名片按群动态核对称呼，称呼用于他人时驳回。
- 鉴权审核注入参与者身份画像与人际关系上下文，提供跨核对依据。
- 鉴权结果事件（通过/驳回/异常放行）接入麦麦观察展示鉴权卡片。
- 新增输入注入检测（防越狱/破甲）：内置六类注入规则（指令覆盖、角色劫持、系统标记伪造、越狱尝试、元指令、多语言绕过）零成本快速检测，规则命中后由独立 LLM 结合聊天上下文确认，降低闲聊误报。
- 输入注入动作策略：注入安全警告（warn_context，让麦麦忽略注入指令）、直接丢弃消息（delete）、仅记录（detect_only）；支持配置自定义关键词与正则。
- 输入注入检测事件（auth.input_injection）接入麦麦观察。
- 新增用户权限分级：按平台+用户ID绑定角色（owner/admin/trusted/normal/blacklist），黑名单用户消息被直接丢弃，不进入对话与记忆。
...
- WebUI 模型测试不再硬编码 temperature=0.0，改为按「模型级温度 → 任务默认温度」解析，测试结果与真实调用一致（部分模型如 Kimi K2.6 只接受 temperature=1）。
- 修复 Gemini 客户端对 Gemma 等开放模型发送 thinking_budget 参数导致 API 报"参数不正确"（400）的问题：不支持思考预算的模型自动省略该参数，Gemini 系列行为不变。
- 新增「网络」配置节：支持为模型 API 等出站请求配置代理地址（http/https/socks5）与直连地址列表（国内厂商域名可直连不绕路），WebUI 可直接编辑，保存后热重载自动生效。
- 模型厂商支持单独配置代理（厂商级优先于全局）：配置后该厂商的全部请求（对话、模型列表获取、测试连接）强制走该代理，不再有直连旁路。
