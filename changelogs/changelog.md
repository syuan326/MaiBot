...
- 鉴权器新增固定身份规则（`auth.identity_rules`）：可将指定用户（平台+用户ID）与一个或多个专属称呼永久绑定（例如某用户是唯一的"哥哥"）；鉴权前由代码做系统级 UID 硬比对（Planner 以当前/回复目标消息发送者为准，Replyer 以被回复消息发送者为准），比对结论与禁止称呼清单动态注入审核 Prompt，称呼被用于他人时驳回；名字无需配置，按 user_id 自动并集已知昵称与各群名片。
- 鉴权驳回反馈强化：Planner/Replyer 被驳回后重新规划/重新生成时，反馈中以醒目格式附带驳回原因与固定身份规则提醒。
- 鉴权决策区分"审核通过"与"审核异常放行"：审核模型调用异常时标记 audit_error，监控事件与卡片可区分两种状态。

## Webui

- 麦麦观察的鉴权卡片从"仅驳回时显示"改为全状态显示：通过（绿色紧凑）、驳回（红/琥珀）、审核异常放行（灰色）三态；卡片展示鉴权识别到的用户及其目标用户判定与相关称呼；时间线新增"鉴权通过"过滤开关，配置项 `auth.emit_passed_events` 可彻底关闭通过事件的产生。
...
- 修复"启用关系向量回填"开关不生效的问题：运行时缺少关系向量回填后台循环，开启后现在会持续为存量关系补写向量。
- 修复热应用检索配置后"启用关系向量"开关不刷新的问题，无需重启即可生效。

## MaiSaka

- 修复 query_memory 工具在 hybrid 模式未提供时间范围时必然检索失败的问题：自动降级为关键词检索并提示；time 模式缺少时间范围时提前明确报错，工具描述同步说明 time/hybrid 必须提供时间参数
