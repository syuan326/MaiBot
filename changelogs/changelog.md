...
- 人格配置新增固定身份规则（`personality.fixed_identities`）：可将指定用户（平台+用户ID）与一个或多个专属称呼永久绑定（例如某用户是唯一的"哥哥"）；规则全局注入 Planner 与 Replyer 提示词，禁止专属称呼用于其他任何人，不可被对话内容或人物画像覆盖。

## MaiSaka

- 鉴权审核注入固定身份规则：配置级专属称呼绑定作为权威身份依据提供给审核模型，Replyer/Planner 输出中把专属称呼用于他人时会被身份矛盾检查驳回。
